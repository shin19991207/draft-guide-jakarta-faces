...
automated tests are a much better approach because they are more reliable and trigger a failure if a breaking change is introduced.
...
The [hotspot=testDataIntegrity file=0]`testDataIntegrity()` test case verifies that each `SystemLoadData` entry in the list of system load data contains valid values for `time`, `loadAverage`, and `memoryUsage`.
